...
int keyMillis=0;
 ...
 LRESULT CALLBACK keyboardHookProc(int nCode, WPARAM wParam, LPARAM lParam)
 {
    ...
    if (p->flags == 0)
    {
        keyMillis = p->time - ofGetStartTime();
        keyCode = (int) p->vkCode;
        keyDetected = true;
        cout << "keystroke detected at time: " << keyMillis << "\n";
        //cout << "elapsed time: " << keyMillis;
    }
		...
		switch (p->vkCode)
		{
            ...
            */
			...
			default:
                ...
                //recorder.recordPress(p->vkCode, lastMillis );
		}
	}
...
//ofSetLogLevel(OF_LOG_VERBOSE);
	...
	this->args->printOpts();
	isCalibrating = false;
	if (this->args->contains("-calibrate"))
	{
	    isCalibrating = true;
	    cout << "starting in calibration mode \n";
	}
	//ofSetLogLevel( OF_LOG_VERBOSE );
	//translationFileName="translations_en.txt";
	translationFileName="translations_nl.txt";
    ofSetWindowTitle("in1klap");
    HWND handleWindow;
    handleWindow = FindWindowA("in1klap", NULL);
   // HINSTANCE handleInstance = (HINSTANCE)GetWindowLong( handleWindow , GWL_HINSTANCE);
    HINSTANCE handleInstance = (HINSTANCE)GetModuleHandle(NULL);
    HHOOK keyboardHook = SetWindowsHookEx(
		WH_KEYBOARD_LL,
		keyboardHookProc,
		handleInstance,
		0);
    cout << handleWindow <<", " << handleInstance << ", " << "\n";
    ...
    ofSetFrameRate(60);
	mainFont.loadFont("arial.ttf",10,true,true);
	ofEnableSmoothing();
    lastMillis = ofGetElapsedTimeMillis();
    lastMils = 0.0f;
  //  ofAddListener(TO.myEventsObject.newFloatEvent, this, &in1klap::onThreadUpdate);
	//TO.start();
	...
	//ofSoundStreamSetup(1,0,this,44100,128,1);
	latency = readLatency();
	cout << "latency from file: " << latency << " ms\n";

	for(int i=0;i!=4;i++){
	    ifstream fin; //declare a file stream
		fin.open(ofToDataPath("sequence_"+ofToString(i)+".txt",1).c_str() ); //open your text file
		vector<string> thisSequence;
		if(fin==NULL){
			std::cout << "Failed to open sequence:  " << ofToDataPath("sequence_"+ofToString(i)+".txt",1).c_str() << std::endl;
		}
    ...
    ifstream fin; //declare a file stream
	// Load translations
	fin.open(ofToDataPath(translationFileName,1).c_str() ); //open your text file
	if(fin==NULL){
		std::cout << "Failed to open translation file:  " << ofToDataPath(translationFileName,1).c_str() << std::endl;
	}
...
int in1klap::readLatency()
{// read %APPDATA%/in1klap/latency.txt
    int myLatency;
    ifstream fin; //declare a file stream
    CHAR  buffer[MAX_PATH];
    //find out where %APPDATA% lives on this machine
    //next line was SHGetFolderPathA
    BOOL result = SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, NULL, buffer);
    cout << buffer << "\n";
    string pad;
    pad += buffer;
    pad += "/in1klap//latency.txt";
    cout << pad << "\n";

    pad = "c:\\latency.txt"; //hack

    fin.open((pad).c_str());
    if(fin==NULL){
        std::cout << "Failed to open latency.txt" << std::endl;
        myLatency = 70; //fallback in case file does not exist
    }
    while(fin!=NULL) //as long as theres still text to be read
    {
        string str; //declare a string for storage
...
void in1klap::draw(){
	...
	{
	    char buffer [33];
	    itoa(deviations.size(), buffer, 10);
	    string count = buffer;
        mainFont.drawString("S: Save calibration data", 20, appTextTop + 120);//Save calibration data
        mainFont.drawString("Calibration Passes:" + count, 20, appTextTop + 140);
        if (deviations.size() > 0)
        {
            double average = 0.0;
            average = std::accumulate(deviations.begin(), deviations.end(), 0.0) / deviations.size();
            //cout << average << "\n";
            //cout << (int) average << "\n";
            //cout << deviations[0] << "\n";
            char buffer2 [33];
            itoa(average, buffer2, 10);
            string averageString = buffer2;
            mainFont.drawString("Average deviation: " + averageString, 20, appTextTop + 160);
        }
	}
	ofSetColor(255,255,255);
	//logo.draw(-16,0,160,160);
    logo.draw(16,16, logo.width +16, logo.height +16);
...
void in1klap::keyPressed(int key)
{
    ...
    {
        if (key == 's')
        {// save measured latency in ms to %APPDATA%/in1klap/latency.txt
            if ( deviations.size() > 0)
            {
                ...
                average = std::accumulate(deviations.begin(), deviations.end(), 0.0) / deviations.size();
                int newLatency = latency + average;
                latency = newLatency;

                ofstream fout; //declare a file stream
                CHAR  buffer[MAX_PATH];
                BOOL result = SHGetFolderPathA(NULL, CSIDL_APPDATA, NULL, NULL, buffer);
                cout << buffer << "\n";
                string pad;
                pad += buffer;
                pad += "/in1klap//latency.txt";
                ...
                if(fout==NULL)
                {
                    std::cout << "Failed to open latency.txt for writing" << std::endl;
                }

                char buffer2 [33];
                itoa(newLatency, buffer2, 10);
                fout << newLatency;
                fout.close();
                string averageString = buffer2;
            }
    ...
    //recorder.recordPress(key, lastMillis ); //+ (int) 128/44.100f  try it, will be stored if active
	if(key=='r'){
		recorder.stop();
		metro.stop();
		appState=0;
...
void in1klap::audioRequested(float *output, int bufferSize, int numChannels)
{
 //   cout << "bufferSize: "<< bufferSize << "\nnumChans: " << numChannels << "\n";
    //cout << ofGetElapsedTimeMillis() - lastMillis << "\n";

    if (keyDetected)
    {
        keyDetected = false;
        recorder.recordPress(keyCode, keyMillis - latency ); //adjust latency
    }
    //cout << lastMillis - ofGetElapsedTimeMillis() << "\n";

    lastMillis = ofGetElapsedTimeMillis();// - (int) (bufferSize * 44.100f) ;
  //  cout << "lastMillis: " << lastMillis << ", perfCounter: " << ((float)cputime.QuadPart/(float)ticksPerSecond.QuadPart)*1000 << "\n";
    audioTimer++;
    //lastMillis = audioTimer * 128;
    //float audioMillis = (audioTimer * bufferSize *3) / 44.100f;
   // cout << "lastMillis: " << lastMillis << ", audioMillis: " << audioMillis<< " difference: " << audioMillis - lastMillis <<"\n";

	lastMils = ofGetElapsedTimeMillis();// - (bufferSize * 44.100f) ;//+

    metro.update(lastMillis);

	//metro.update();

//	recorder.audioTimer();
	//cout << bufferSize << " " << numChannels << "\n";
	/*
	for(int i = 0; i!=128; i++) {
...
pair<string, double> in1klap::calculateResult(const vector<pair<int, string> >& expected, vector<pair<int, string> >& received, const vector<int>& beatvector, bool anyKey){
	...
	compensate(received, expected); //hack
...
void in1klap::compensate(vector<pair<int, string> >& received, const vector<pair<int, string> >& expected){
	...
	if(differences.size() > 0){
        ...
        if ( isCalibrating ) deviations.push_back ( meanDeviationFromExpected );
...
//
//EVERYTHING UNDER THIS LINE IS DEPRECATED / UNUSED
//
//_____________________________________________________________________
...
void in1klap::onThreadUpdate(float &myFloat){

  //  int deltaTime = ofGetElapsedTimeMillis() - lastMillis;
//	cout << "received thread tick after " << myFloat << "ms \n";
	//lastMillis = ofGetElapsedTimeMillis();
	//for (int i = 0; i<deltaTime; i++)
	//{
        metro.update(myFloat);
        //recorder.audioTimer();
        lastMillis = (int) myFloat;
	//}
	//metro.update();
//	cout << ofGetElapsedTimeMillis() - lastMillis << "\n";
/*
...
/*
void CALLBACK in1klap::TimerFunction(UINT wTimerID, UINT msg,
