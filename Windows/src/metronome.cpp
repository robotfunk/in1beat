#include "metronome.h"
metronome::metronome() : bpm(120), flipFlop(0), samplePos(0),lengthOfOneBeatInSamples(0), active(false){
	recalculateBeatLengths();
	tick.loadSound("metronome2.aif");
	tock.loadSound("metronome1.aif");
	lastMillis =0;
	roundingError = 0.0f;
}

void metronome::start(){
	samplePos=0;
	beatCounter=0;
	active=true;
	tickGo=false;
	tockGo=false;
	startMillis = ofGetElapsedTimeMillis();
}

bool metronome::isActive(){
	return(active);
}

void metronome::stop(){
	active=false;
	float testTime = ofGetElapsedTimeMillis() - startMillis;
	cout << "this test took " << testTime << " milliseconds \n";
	cout << "during which we had " << roundingError << " ms of rounding errors \n";
	roundErrorPercentage = roundingError / testTime;
	cout << "which means " << roundErrorPercentage*100 << " percent \n";
}

// this variant is updated from threadedObject
void metronome::update(float ms){
  //  cout << "begin: " << ofGetElapsedTimeMillis() << "\n

  //cout << (ms-lastMillis)*44.1;
//  cout << ms << "\n";
//  int elapsed = (ms*1000 - lastMillis*1000

  	float elapsedF= (ms - lastMillis)*44.100f; //-roundingError nieuw 20110213
  	int elapsed = (int) (elapsedF + .5);
  	roundingError += elapsedF - elapsed;


  	//float roundingError = elapsedF - elapsed;
  //	cout << roundingError<<"\n";
  	lastMillis = ms;
  	/*
  	if (roundingError<1)//sometimes gives infinite? do not want
  	{
        lastMillis =ms+roundingError;//-roundingError;
  	}
  	else
  	{
  	    lastMillis = ms;
  	}*/
	if (!active || lengthOfOneBeatInSamples==0){
//	    cout << "not doing anything\n";
	//    cout << "end: " << ofGetElapsedTimeMillis() << "\n";
		return;
	}

	for(int i = 0; i<elapsed; i++) {

	//}
	//for(int i = 0; i!=128; i++) {
		if(samplePos % (lengthOfOneBeatInSamples*4) == 0.0) {
			tickGo=true; // resets the tick sound to the beginning, so that its audio will be added to the output
		}
		else if(samplePos % lengthOfOneBeatInSamples == 0.0) {
			tockGo=true; // resets the tock sound to the beginning, so that its audio will be added to the output
		}
		samplePos++; // this gets incremented with every sample
		//output[i*2] = output[i*2+1] = 0;
	}
	if(tickGo){
		if(beatCounter>35){
			stop();
			return;
		};
		tick.play();
		tickGo=false;
		flipFlop=!flipFlop;
		beatCounter++;
		std::cout<<"Beatcounter="<<beatCounter<<std::endl;

	}
	else if(tockGo){
		if(beatCounter>35){
			stop();
			return;
		};
		tock.play();
		tockGo=false;
		flipFlop=!flipFlop;
		beatCounter++;
		std::cout<<"Beatcounter="<<beatCounter<<std::endl;

	}

 //cout << "end: " << ofGetElapsedTimeMillis() << "\n";
}

// this variant gets called from the testapp audio requested method (ie. once per frame)
void metronome::update(){
//cout << ofGetElapsedTimeMillis() << "\n";
//cout << "metro.update() [SHOULD NOT BE USED I THINK] \n";
	if (!active || lengthOfOneBeatInSamples==0){
//	    cout << "not doing anything\n";
	//    cout << "end: " << ofGetElapsedTimeMillis() << "\n";
		return;
	}


	for(int i = 0; i!=128; i++) {
		if(samplePos % (lengthOfOneBeatInSamples*4) == 0.0) {
			tickGo=true; // resets the tick sound to the beginning, so that its audio will be added to the output
		}
		else if(samplePos % lengthOfOneBeatInSamples == 0.0) {
			tockGo=true; // resets the tock sound to the beginning, so that its audio will be added to the output
		}
		samplePos++; // this gets incremented with every sample
		//output[i*2] = output[i*2+1] = 0;
	}
	if(tickGo){
		if(beatCounter>35){
			stop();
			return;
		};
		tick.play();
		tickGo=false;
		flipFlop=!flipFlop;
		beatCounter++;
		std::cout<<"Beatcounter="<<beatCounter<<std::endl;

	}
	else if(tockGo){
		if(beatCounter>35){
			stop();
			return;
		};
		tock.play();
		tockGo=false;
		flipFlop=!flipFlop;
		beatCounter++;
		std::cout<<"Beatcounter="<<beatCounter<<std::endl;

	}

 //cout << "end: " << ofGetElapsedTimeMillis() << "\n";
}


void metronome::recalculateBeatLengths(){
	int samplespersecond=44100;
	double beatspersecond=bpm/60.0; // 2
	double secondsperbeat=1/beatspersecond; // 0.5
	lengthOfOneBeatInSamples=secondsperbeat*samplespersecond;
}

bool metronome::getFlipFlop(){
	return(flipFlop);
}
int metronome::getBeatNumber(){
	return(beatCounter);
}

void metronome::setBpm(int targetBpm){
	bpm=targetBpm;
	recalculateBeatLengths();
}

int metronome::getBpm(){
	return(bpm);
}

int metronome::lengthOf16thInMs(){
	return(((lengthOfOneBeatInSamples/4.0)/44100.0)*1000.0);
}

